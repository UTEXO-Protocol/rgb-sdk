...
export const PROXY_HTTP_URL =
  process.env.SIGNET_PROXY_HTTP_URL || 'https://rgb-proxy-utexo.utexo.com/json-rpc';
...
export type UTEXOWalletType = {
  ...
  blindReceive(params: { amount: number; minConfirmations: number }): Promise<InvoiceData>;
  witnessReceive(params: { amount: number; minConfirmations: number }): Promise<InvoiceData>;
...
export async function setupSignetPreflight(state: PreflightState): Promise<void> {
  env('MNEMONIC_A');
  env('MNEMONIC_B');
  env('ASSET_ID');
  ...
  state.receiverBalanceBefore = await receiver.getAssetBalance(assetId).catch(() => ({
    settled: 0,
    future: 0,
    spendable: 0,
    offchainOutbound: 0,
...
export async function disposeSignetPreflight(state: PreflightState): Promise<void> {
...
export async function finalizeTransferSnapshot(
  ...
  try {
    ...
    const currentTransferFromSnapshot = Array.isArray(report.phase2.listTransfersSnapshot)
      ? report.phase2.listTransfersSnapshot.find((item) => item.recipientId === recipientId)
      : undefined;
    ...
    if (currentTransferFromSnapshot) {
      ...
      report.phase1.txidMatch = Boolean(
        currentTransferFromSnapshot.txid && currentTransferFromSnapshot.txid === sendTxid,
      );
...
export async function runSignetReceiveSmoke(params: {
  ...
  try {
    ...
    for (let cycle = 1; cycle <= IDEMPOTENT_REFRESH_COUNT; cycle += 1) {
      ...
      report.phase2.refreshChecks.push({ cycle, settled: Number(balance.settled) });
    }
