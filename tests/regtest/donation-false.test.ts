...
beforeAll(async () => {
  ...
  const issuedAsset = await sender.issueAssetNia({
    ticker: `D${Date.now().toString().slice(-5)}`,
    name: `Donate${Date.now().toString().slice(-5)}`,
    amounts: [10],
    precision: 0,
  });
...
describe('Regtest donation=false flow', () => {
  it('broadcasts only after the receiver-side ACK path and sender refresh run', async () => {
    const sender = state.sender!;
    const receiver = state.receiver!;
    const startedAt = Date.now();
    const report: DonationFalseReport = {
